...
fun TrailingIcon(isChecking: Boolean, isValid: Boolean, errorMessage: String) {
    if (isChecking) CircularProgressIndicator(
        modifier = Modifier.size(TextFieldTokens.ProgressSize),
        strokeWidth = TextFieldTokens.ProgressStrokeWidth,
        color = DesignTheme.colors.info
    )
    else if (isValid) Icon(Icons.Default.Check, null, tint = DesignTheme.colors.info)
    else if (errorMessage.isNotEmpty()) Icon(
...
object StyledInputField {
    ...
    operator fun invoke(
        ...
        focusedBorderColor: Color = DesignTheme.colors.info,
        unfocusedBorderColor: Color = DesignTheme.colors.disabledContainer,
        focusedContainerColor: Color = DesignTheme.colors.background,
        unfocusedContainerColor: Color = DesignTheme.colors.surfaceVariant.copy(alpha = 0.5f),
        cursorColor: Color = DesignTheme.colors.info,
